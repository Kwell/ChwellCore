...
TEST(BenchmarkTest, VectorAlloc) {
    ...
    suite.add_benchmark("vector_alloc_10000", "Vector of 10000 ints", []() {
        memory_bench::benchmark_vector_alloc(10000, 10000);
    });
...
TEST(BenchmarkTest, StringConcat) {
    ...
    suite.add_benchmark("string_concat_1000", "String concat 1000 times", []() {
        memory_bench::benchmark_string_concat(1000);
    });
...
TEST(BenchmarkTest, MapInsert) {
    ...
    suite.add_benchmark("map_insert_1000", "Map insert 1000 items", []() {
        memory_bench::benchmark_map_insert(1000);
    });
...
// 与 src/benchmark/benchmark.cpp 中 tcp_bench 实现一致
TEST(BenchmarkTest, LibraryTcpBench) {
    BenchmarkSuite suite("TCP socketpair microbench");

    suite.add_benchmark("tcp_connect", "socketpair open/close", []() {
        tcp_bench::benchmark_tcp_connect();
    });
    suite.add_benchmark("tcp_send_recv", "socketpair send/recv", []() {
        tcp_bench::benchmark_tcp_send_receive();
    });
    suite.add_benchmark("tcp_many_pairs", "many socketpairs", []() {
        tcp_bench::benchmark_concurrent_connections(32);
    });
...
// 与 src/benchmark/benchmark.cpp 中 loadbalance_bench 实现一致
TEST(BenchmarkTest, LibraryLoadBalanceBench) {
    BenchmarkSuite suite("Load balance microbench");

    suite.add_benchmark("lb_round_robin", "RoundRobin select", []() {
        loadbalance_bench::benchmark_round_robin_select(200);
    });
    suite.add_benchmark("lb_consistent_hash", "ConsistentHash select", []() {
        loadbalance_bench::benchmark_consistent_hash_select(200);
    });
    suite.add_benchmark("lb_weighted_rr", "WeightedRoundRobin select", []() {
        loadbalance_bench::benchmark_weighted_round_robin_select(200);
    });
