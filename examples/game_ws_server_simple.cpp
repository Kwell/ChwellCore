...
struct Room {
  string room_id;
  unordered_set<int> connection_ids;
};
...
private:
  ...
  void client_loop(int client_fd) {
    while (running_) {
      ...
      // 简单解析 JSON（假设是 { "type": "login", ... } 格式）
      if (payload.find("\"type\":\"login\"") != std::string::npos) {
        handleLogin(client_fd, payload);
      } else if (payload.find("\"type\":\"chat\"") != std::string::npos) {
  ...
  void handleLogin(int client_fd, const std::string& payload) {
    // 简单解析 player_id（查找最后一个引号内的内容）
    size_t start = payload.find(": \"") + 3;
    size_t end = payload.find("\", start);
    if (start == std::string::npos || end == std::string::npos) {
      std::cerr << "Failed to parse playerId" << std::endl;
      return;
    }

    std::string player_id = payload.substr(start, end - start);
    std::cout << "Login request: player_id=" << player_id << std::endl;
    ...
    // 发送登录成功响应（JSON）
    std::string response = "{\"type\":\"login\",\"ok\":true,\"message\":\"Login success\"}";
    ws_write_text_frame(client_fd, response.data(), response.length());
  }

  void handleChat(int client_fd, const std::string& payload) {
    // 检查是否已登录
    ...
    std::cout << "Chat message: content=" << content << std::endl;
    ...
    // 广播消息到房间（JSON）
    std::string response = "{\"type\":\"chat\",\"fromPlayerId\":\"" + player.player_id + "\",\"content\":\"" + content + "\"}";
    broadcastChat("main", response);
  }

  void broadcastChat(const std::string& room_id, const std::string& message) {
    ...
    // 广播到房间内的所有连接
    for (int fd : room.connection_ids) {
      ws_write_text_frame(fd, message.data(), message.length());
    }

    std::cout << "Broadcasted chat to " << room.connection_ids.size() << " connections" << std::endl;
  }
  ...
  void on_disconnect(int client_fd) {
    ...
    if (pit != players_.end()) {
      ...
      if (rit != rooms_.end()) {
        Room& room = rit->second;
        room.connection_ids.erase(client_fd);

        // 如果房间空了，删除房间
        if (room.connection_ids.empty()) {
          rooms_.erase(rit);
...
private:
  ...
  unordered_map<std::string, Room> rooms_;
};
