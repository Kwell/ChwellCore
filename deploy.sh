...
# SERVICE_BINARY   - 要启动的服务: example_http_server / example_protocol_server，缺省: example_http_server
...
for arg in "$@"; do
  case "$arg" in
    --build|-b)
      ...
      ;;
    --help|-h)
      echo "Usage: $0 [example_http_server|example_protocol_server] [--build|-b]"
      echo ""
      echo "Options:"
      ...
      echo "  $0 example_protocol_server  # 启动 protocol 服务"
      echo "  $0 --build                  # 强制重新构建后部署"
      echo "  $0 example_protocol_server --build  # 重新构建并启动 protocol 服务"
      exit 0
      ;;
    example_http_server|example_protocol_server)
      SERVICE_BINARY="$arg"
      ;;
    *)
...
echo ">>> Running container: $CONTAINER_NAME (service=$SERVICE_BINARY, http=$HTTP_PORT, tcp=$TCP_PORT)"
docker run -d \
  --name "$CONTAINER_NAME" \
